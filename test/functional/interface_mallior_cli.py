...
# The block reward of coinbaseoutput.nValue (50) MLR/block matures after
# COINBASE_MATURITY (100) blocks. Therefore, after mining 101 blocks we expect
# node 0 to have a balance of (BLOCKS - COINBASE_MATURITY) * 50 MLR/block.
...
class TestMalliorCli(MalliorTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Compare responses from getblockchaininfo RPC and `mallior-cli getblockchaininfo`")
        ...
        self.log.info("Test -getinfo and mallior-cli return all proxies")
        ...
        if self.is_specified_wallet_compiled():
            self.log.info("Test -getinfo and mallior-cli getwalletinfo return expected wallet info")
            ...
            assert_equal(Decimal(cli_get_info['Transaction fee rate (-paytxfee) (MLR/kvB)']), wallet_info['paytxfee'])
            assert_equal(Decimal(cli_get_info['Min tx relay fee rate (MLR/kvB)']), network_info['relayfee'])
            ...
            # Mine a block to confirm; adds a block reward (50 MLR) to the default wallet.
            ...
            # Test mallior-cli -generate with -rpcwallet in multiwallet mode.
