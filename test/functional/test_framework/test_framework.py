...
TMPDIR_PREFIX = "mallior_func_test_"
...
class MalliorTestMetaClass(type):
    """Metaclass for MalliorTestFramework.

    Ensures that any attempt to register a subclass of `MalliorTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'MalliorTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("MalliorTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("MalliorTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class MalliorTestFramework(metaclass=MalliorTestMetaClass):
    """Base class for a mallior test script.

    Individual mallior test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave malliords and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop malliords after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use mallior-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_malliord = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "malliord" + config["environment"]["EXEEXT"],
        )
        fname_malliorcli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "mallior-cli" + config["environment"]["EXEEXT"],
        )
        fname_malliorutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "mallior-util" + config["environment"]["EXEEXT"],
        )
        self.options.malliord = os.getenv("MALLIORD", default=fname_malliord)
        self.options.malliorcli = os.getenv("MALLIORCLI", default=fname_malliorcli)
        self.options.malliorutil = os.getenv("MALLIORUTIL", default=fname_malliorutil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: malliords were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("MalliorRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'malliord', self.options.malliord) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'mallior-cli', self.options.malliorcli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a malliord test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple malliord test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as malliord's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    malliord=self.options.malliord,
                    mallior_cli=self.options.malliorcli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in mallior.conf
    ...
    def skip_if_no_malliord_tracepoints(self):
        """Skip the running test if malliord has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("malliord has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_malliord_zmq(self):
        """Skip the running test if malliord has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("malliord has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if mallior-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("mallior-wallet has not been compiled")

    def skip_if_no_mallior_util(self):
        """Skip the running test if mallior-util has not been compiled."""
        if not self.is_mallior_util_compiled():
            raise SkipTest("mallior-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if mallior-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("mallior-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether mallior-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether mallior-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_mallior_util_compiled(self):
        """Checks whether mallior-util was compiled."""
        return self.config["components"].getboolean("ENABLE_MALLIOR_UTIL")
