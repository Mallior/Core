# Bootstrappable Mallior Core Builds

This directory contains the files necessary to perform bootstrappable Mallior
Core builds.
...
To build Mallior Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Mallior Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/mallior-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/mallior-core/guix.sigs).
