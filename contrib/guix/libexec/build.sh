...
(
    ...
    case "$HOST" in
        *mingw*)
            make deploy ${V:+V=1} MALLIOR_WIN_INSTALLER="${OUTDIR}/${DISTNAME}-win64-setup-unsigned.exe"
            ;;
    esac

    # Setup the directory where our Mallior Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built Mallior Core to $INSTALLPATH
    case "$HOST" in
        *darwin*)
    ...
    (
        ...
        # copy over the example mallior.conf file. if contrib/devtools/gen-mallior-conf.sh
        # has not been run before buildling, this file will be a stub
        cp "${DISTSRC}/share/examples/mallior.conf" "${DISTNAME}/"
