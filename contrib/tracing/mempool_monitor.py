...
""" Example logging Mallior Core mempool events using the mempool:added,
...
def main(malliord_path):
    malliord_with_usdts = USDT(path=str(malliord_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    malliord_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    malliord_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    malliord_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    malliord_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[malliord_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/malliord")
        exit(1)
