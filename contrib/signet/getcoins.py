...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing mallior-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='mallior-cli', help='mallior-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Mallior address to which the faucet should send')
...
parser.add_argument('mallior_cli_args', nargs='*', help='Arguments to pass on to mallior-cli (default: -signet)')

args = parser.parse_args()

if args.mallior_cli_args == []:
    args.mallior_cli_args = ['-signet']


def mallior_cli(rpc_command_and_params):
    argv = [args.cmd] + args.mallior_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = mallior_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = mallior_cli(['getnewaddress', 'faucet', 'bech32'])
