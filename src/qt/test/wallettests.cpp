                  ...
                  QMessageBox::StandardButton confirm_type = QMessageBox::Yes)
{
    ...
    entry->findChild<MalliorAmountField*>("payAmount")->setValue(amount);
...
void CompareBalance(WalletModel& walletModel, CAmount expected_balance, QLabel* balance_label_to_check)
{
    ...
    QString balanceComparison = MalliorUnits::formatWithUnit(unit, expected_balance, false, MalliorUnits::SeparatorStyle::ALWAYS);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_mallior-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_mallior-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_mallior-qt  # macOS
void TestGUI(interfaces::Node& node, const std::shared_ptr<CWallet>& wallet)
{
    ...
    // Amount input
    MalliorAmountField* amountInput = receiveCoinsDialog.findChild<MalliorAmountField*>("reqAmount");
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_mallior-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
