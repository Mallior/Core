...
#include <qt/mallioraddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Mallior address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new MalliorAddressEntryValidator(parent));
    widget->setCheckValidator(new MalliorAddressCheckValidator(parent));
}
...
bool parseMalliorURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no mallior: URI
    if(!uri.isValid() || uri.scheme() != QString("mallior"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!MalliorUnits::parse(MalliorUnit::MLR, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseMalliorURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseMalliorURI(uriInstance, out);
}

QString formatMalliorURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("mallior:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(MalliorUnits::format(MalliorUnit::MLR, info.amount, false, MalliorUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openMalliorConf()
{
    ...
    /* Open mallior.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Mallior.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Mallior (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Mallior (%s).lnk", chain));
}

bool GetStartOnSystemStartup()
{
    // check for Mallior*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "mallior.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("mallior-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a mallior.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Mallior\n";
        else
            optionFile << strprintf("Name=Mallior (%s)\n", chain);
