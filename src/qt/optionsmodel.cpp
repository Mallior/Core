...
/** Call node.updateRwSetting() with Mallior 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Mallior 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/mallior/mallior/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because mallior 22.x will not try to read these.
...
//! Convert enabled/size values to mallior -prune setting.
...
//! Get pruning enabled value to show in GUI from mallior -prune setting.
...
//! Get pruning size value to show in GUI from mallior -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayMalliorUnit")) {
        settings.setValue("DisplayMalliorUnit", QVariant::fromValue(MalliorUnit::MLR));
    }
    QVariant unit = settings.value("DisplayMalliorUnit");
    if (unit.canConvert<MalliorUnit>()) {
        m_display_mallior_unit = unit.value<MalliorUnit>();
    } else {
        m_display_mallior_unit = MalliorUnit::MLR;
        settings.setValue("DisplayMalliorUnit", QVariant::fromValue(m_display_mallior_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if mallior.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_mallior_unit);
...
{
    if (new_unit.isNull() || new_unit.value<MalliorUnit>() == m_display_mallior_unit) return;
    m_display_mallior_unit = new_unit.value<MalliorUnit>();
    QSettings settings;
    settings.setValue("DisplayMalliorUnit", QVariant::fromValue(m_display_mallior_unit));
    Q_EMIT displayUnitChanged(m_display_mallior_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/mallior/mallior/pull/8273
    ...
    // (https://github.com/mallior-core/gui/issues/567).
