...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<MalliorUnit>("MalliorUnit");
#else
    qRegisterMetaType<MalliorUnit>("MalliorUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. mallior_de.qm (shortcut "de" needs to be defined in mallior.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. mallior_de_DE.qm (shortcut "de_DE" needs to be defined in mallior.qrc)
...
static const char* qt_argv = "mallior-qt";

MalliorApplication::MalliorApplication()
...
void MalliorApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the MalliorApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", MalliorGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void MalliorApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool MalliorApplication::createOptionsModel(bool resetSettings)
{
...
void MalliorApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new MalliorGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &MalliorGUI::quitRequested, this, &MalliorApplication::requestShutdown);
...
void MalliorApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void MalliorApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool MalliorApplication::baseInitialize()
{
    return node().baseInitialize();
}

void MalliorApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &MalliorApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &MalliorApplication::handleRunawayException);
    connect(this, &MalliorApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &MalliorApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void MalliorApplication::parameterSetup()
{
...
void MalliorApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void MalliorApplication::requestInitialize()
{
...
void MalliorApplication::requestShutdown()
{
...
void MalliorApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        // mallior: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &MalliorGUI::handlePaymentRequest);
            connect(window, &MalliorGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void MalliorApplication::handleRunawayException(const QString &message)
{
...
void MalliorApplication::handleNonFatalException(const QString& message)
{
...
WId MalliorApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool MalliorApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", MalliorGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(mallior);
    Q_INIT_RESOURCE(mallior_locale);
    ...
    /// 6-7. Parse mallior.conf, determine network, switch to network specific
    ...
    // mallior: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
