...
#include <qt/malliorgui.h>

#include <qt/malliorunits.h>
...
const std::string MalliorGUI::DEFAULT_UIPLATFORM =
...
MalliorGUI::MalliorGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &MalliorGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &MalliorGUI::showModalOverlay);
...
void MalliorGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Mallior address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and mallior: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &MalliorGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &MalliorGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &MalliorGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Mallior addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Mallior addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Mallior Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Mallior Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a mallior: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Mallior command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &MalliorGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &MalliorGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &MalliorGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &MalliorGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &MalliorGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &MalliorGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &MalliorGUI::setCurrentWallet, Qt::QueuedConnection);
                    activity->open(path);
                });
            }
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &MalliorGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &CreateWalletActivity::created, this, &MalliorGUI::setCurrentWallet);
            activity->create();
        });
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_mask_values_action, &QAction::toggled, this, &MalliorGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &MalliorGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &MalliorGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &MalliorGUI::showDebugWindow);
}

void MalliorGUI::createMenuBar()
{
#ifdef Q_OS_MACOS
...
void MalliorGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &MalliorGUI::setCurrentWalletBySelectorIndex);
...
void MalliorGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &MalliorGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &MalliorGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &MalliorGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &MalliorGUI::showProgress);
...
#ifdef ENABLE_WALLET
void MalliorGUI::enableHistoryAction(bool privacy)
{
    historyAction->setEnabled(!privacy);
    if (historyAction->isChecked()) gotoOverviewPage();
}

void MalliorGUI::setWalletController(WalletController* wallet_controller)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &MalliorGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &MalliorGUI::removeWallet);
...
WalletController* MalliorGUI::getWalletController()
{
    return m_wallet_controller;
}

void MalliorGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &MalliorGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &MalliorGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &MalliorGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &MalliorGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &MalliorGUI::incomingTransaction);
    connect(this, &MalliorGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void MalliorGUI::removeWallet(WalletModel* walletModel)
{
...
void MalliorGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void MalliorGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void MalliorGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void MalliorGUI::setWalletActionsEnabled(bool enabled)
{
...
void MalliorGUI::createTrayIcon()
{
...
void MalliorGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &MalliorGUI::toggleHidden);
...
void MalliorGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void MalliorGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void MalliorGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void MalliorGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void MalliorGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void MalliorGUI::openClicked()
{
...
void MalliorGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void MalliorGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void MalliorGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void MalliorGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void MalliorGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void MalliorGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void MalliorGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void MalliorGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Mallior network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void MalliorGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void MalliorGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void MalliorGUI::updateHeadersSyncProgressLabel()
{
...
void MalliorGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void MalliorGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &MalliorGUI::quitRequested);
...
void MalliorGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void MalliorGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void MalliorGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &MalliorGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &MalliorGUI::show);
                e->ignore();
            }
        }
    }
...
void MalliorGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void MalliorGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void MalliorGUI::incomingTransaction(const QString& date, MalliorUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(MalliorUnits::formatWithUnit(unit, amount, true));
...
void MalliorGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void MalliorGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool MalliorGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool MalliorGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void MalliorGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void MalliorGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void MalliorGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void MalliorGUI::updateProxyIcon()
{
    std::string ip_port;
...
void MalliorGUI::updateWindowTitle()
{
...
void MalliorGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void MalliorGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void MalliorGUI::detectShutdown()
{
...
void MalliorGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void MalliorGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(MalliorGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = MalliorGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void MalliorGUI::subscribeToCoreSignals()
{
...
void MalliorGUI::unsubscribeFromCoreSignals()
{
...
bool MalliorGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<MalliorUnit> units = MalliorUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const MalliorUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, MalliorUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const MalliorUnit u : MalliorUnits::availableUnits()) {
        menu->addAction(MalliorUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(MalliorUnit newUnits)
{
    setText(MalliorUnits::longName(newUnits));
}
