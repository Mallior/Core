...
const char * const MALLIOR_CONF_FILENAME = "mallior.conf";
const char * const MALLIOR_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //mallior-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", MALLIOR_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Mallior
    // macOS: ~/Library/Application Support/Mallior
    // Unix-like: ~/.mallior
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Mallior";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Mallior";
#else
    // Unix-like
    return pathRet / ".mallior";
#endif
#endif
}
...
{
    return GetConfigFile(*this, GetPathArg("-conf", MALLIOR_CONF_FILENAME));
}
