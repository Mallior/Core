# CJDNS support in Mallior Core

It is possible to run Mallior Core over CJDNS, an encrypted IPv6 network that
...
done outside of Mallior Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Mallior network and individual nodes.
...
before it will work with your Mallior Core node. This is described in steps
...
## Run Mallior Core with CJDNS

Once you are connected to the CJDNS network, the following Mallior Core
...
When enabled, this option tells Mallior Core that it is running in an
...
IPv6 local network. This helps Mallior Core perform better address management:
...
CJDNS support was added to Mallior Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `mallior-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/mallior/mallior/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Mallior Core

There are several ways to see your CJDNS address in Mallior Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `mallior-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `mallior-cli getpeerinfo`).
