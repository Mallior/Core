# User-space, Statically Defined Tracing (USDT) for Mallior Core

Mallior Core includes statically defined tracepoints to allow for more
...
**Note**: Mallior Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Mallior Core
...
Multiple tools can list the available tracepoints in a `malliord` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Mallior Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/malliord
stap net        outbound_message 0x0000000000107c05 /src/malliord
stap validation block_connected  0x00000000002fb10c /src/malliord
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Mallior Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/malliord | grep NT_STAPSDT -A 4 -B 2
