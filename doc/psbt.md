# PSBT Howto for Mallior Core

Since Mallior Core 0.17, an RPC interface exists for Partially Signed Mallior
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/mallior/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Mallior transactions that are not fully signed
...
#### Multisig with multiple Mallior Core instances
...
Mallior Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `mallior-cli` is used.
...
Later, when *V* MLR has been received on *Amulti*, and Bob and Carol want to
