...
Below are some notes on how to build Mallior Core for Windows.

The options known to work for building Mallior Core on Windows are:
...
Note that for WSL the Mallior Core source path MUST be somewhere in the default mount file system, for
example /usr/src/mallior, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\mallior`, for example:

    make install DESTDIR=/mnt/c/workspace/mallior
