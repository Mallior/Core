...
The Mallior Core daemon can be configured to act as a trusted "border
router", implementing the mallior wire protocol and relay, making
...
The ZeroMQ feature in Mallior Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building malliord:
...
For instance:
    ...
    $ malliord -zmqpubhashtx=tcp://127.0.0.1:218265 \
               -zmqpubhashtx=tcp://192.168.1.2:218265 \
               -zmqpubhashblock="tcp://[::1]:218266" \
               -zmqpubrawtx=ipc:///tmp/malliord.tx.raw \
               -zmqpubhashtxhwm=10000

Each PUB notification has a topic and body, where the header
corresponds to the notification type. For instance, for the
notification `-zmqpubhashtx` the topic is `hashtx` (no null
terminator). These options can also be provided in mallior.conf.
...
From the perspective of malliord, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into malliord directly. Furthermore, no information is
...
using. Malliord appends an up-counting sequence number to each
