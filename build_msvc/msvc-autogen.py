...
def parse_config_into_mlr_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"Mallior Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/mallior_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/mallior_config.h'), "w", encoding="utf8") as mlr_config:
        mlr_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='Mallior-core msbuild configuration initialiser.')
    ...
    parse_config_into_mlr_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/mallior_config.h'), os.path.join(SOURCE_DIR, 'config/mallior-config.h'))
